...
<body>
      ...
      <div class="container-fluid">
         <div class="row">
            <div class="cabecalho-logado bg-secondary text-white">
               ...
               <div>
                   <a class="botao-sair btn btn-secondary" style="float: right; position: absolute; margin: 0.15% -3rem 0rem" href="logout.php">Sair</a>
   ...
   <div id="resultados" style="margin-top: auto; margin-left: 10%;">
        <?php foreach ($empresas as $empresa): ?>
            <div class="empresa card mb-3">
                <div class="card-body">
                    <h3 class="card-title"><?= $empresa['nome'] ?></h3>
                    <p class="card-text"><i class="fas fa-map-marker-alt"></i> Endereço: <?= $empresa['endereco'] ?></p>
                    <p class="card-text"><i class="fas fa-phone"></i> Telefone: <?= $empresa['telefone'] ?></p>
                    <p class="card-text"><i class="far fa-clock"></i> Horário de Funcionamento: <?= $empresa['horario'] ?></p>
                    <p class="card-text"><i class="fas fa-map-marker-alt"></i> <a href="<?= $empresa['link_google_maps'] ?>" target="_blank">Ver no Google Maps</a></p>
                    <button class="btn btn-primary" onclick="avaliarEmpresa(<?= $empresa['id'] ?>)">Avaliar local</button>
                </div>
            </div>
        <?php endforeach; ?>
    </div>
   ...
   <footer>
      <div id="localSelecionado" style="box-shadow: #a7a7a7 0px 0px 15px; margin-left: 5%; border-radius: 1rem; display: none; width: 90%; background-color: #f2f2f2; padding: 10px; padding-bottom: 1%;">
    ...
    function mostrarResultados(resultados) {
        ...
        if (resultados.length > 0) {
            resultados.forEach(function(empresa) {
                ...
                elementoEmpresa.className = "card mb-3";
                elementoEmpresa.innerHTML = `
                    <div class="card-body">
                        ...
                        <p class="card-text"><i class="fas fa-phone"></i> Telefone: ${empresa.telefone}</p>
                        <p class="card-text"><i class="far fa-clock"></i> Horário de Funcionamento: ${empresa.horario}</p>
                        <p class="card-text"><i class="fas fa-map-marker-alt"></i> <a href="${empresa.link_google_maps}" style="text-decoration: none; color: #000000; font-weight: 700;" target="_blank">Ver no Google Maps</a></p>
                        <button class="btn btn-primary" onclick="avaliarEmpresa(${empresa.id})">Avaliar local</button>
                    </div>
                `;
...
function avaliarEmpresa(idEmpresa) {
    window.location.href = "avaliacao.php?id=" + idEmpresa;
}
</script>
</body>
</html>
