...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $id_pj = $_POST["empresa_id"]; // Ajuste aqui para corresponder ao nome enviado pelo formulário
    $avaliacao = $_POST["avaliacao"];
    $comentario = $_POST["comentario"];

    // Verifica se o ID da empresa existe na tabela pj_registro
    $sql_check = "SELECT * FROM pj_registro WHERE id_pj = '$id_pj'";
    echo "SQL de Verificação: $sql_check <br>"; // Debug: Exibe o SQL de verificação
    $result_check = $conn->query($sql_check);

    var_dump($result_check); // Debug: Exibe o resultado da consulta
    ...
    if ($result_check->num_rows > 0) {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            header("Location: index.php"); // Redirecionamento para index.php após enviar a avaliação
...
$sql_empresas = "SELECT id_pj, nome_empresa FROM pj_registro";
...
if ($result_empresas->num_rows > 0) {
    ...
    while ($row = $result_empresas->fetch_assoc()) {
        echo "<option value='" . $row['id_pj'] . "'>" . $row['nome_empresa'] . "</option>";
    }
